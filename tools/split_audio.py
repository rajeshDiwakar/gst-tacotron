...
def split_audio(audio,caption,root,sr=22050,buffer=1800,augmentation=None,aug_factor=3,min_duration=1,max_duration=10,silence='0',margin='0'):
    ...
    if len(margin)>= 2:
        ...
        lmargin,rmargin=int(margin[0]),int(margin[0])
    ...
    for cap in tqdm(aug_captions):
        ...
        if type(start) == str:
            ...
            start = max(0,str2ms(startstr)-lmargin)
            end = str2ms(endstr)+rmargin
        else:
...
if __name__ == '__main__':
        ...
        split_audio(args.audio,args.caption,args.data_dir,augmentation=args.augment,aug_factor=args.aug_factor,
                    min_duration=args.min_duration,max_duration=args.max_duration,silence=args.silence,margin=args.margin)
