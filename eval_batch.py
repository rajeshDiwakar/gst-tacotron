...
def run_eval(args):
  ...
  if args.reference_audio is not None:
    ...
    style_path = '%s_ref-%s-style' % (base_path, os.path.splitext(os.path.basename(args.reference_audio))[0])
  else:
    if hparams.use_gst:
      print("*******************************")
  ...
  with open(path, 'wb') as f:
    ...
    f.write(synth.synthesize(args.text, mel_targets=mel_targets, reference_mel=reference_mel, alignment_path=alignment_path, style_path=style_path))
